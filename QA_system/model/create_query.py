...
def word_tokenize(text, lang):
    return text.lower().replace('!', ' ! ').replace('?', ' ? ').replace('.', ' . ').replace(',', ' , ').replace('"', '').replace("'", '').replace('|', '').replace(':', '').replace(';', '').replace('(', '').replace(')', '').replace('[', '').replace(']', '').replace('/', '').strip().split()
...
def get_defining_words(words, lang):
    ...
    if lang == 'pl':
        defining_words = [word for word in words if polish_pos.get(word, '') in ['subst', 'adj']]
    else:
